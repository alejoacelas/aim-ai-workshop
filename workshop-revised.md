...
Ask your agent to clone the workshop skills repo and install the skills:

```
git clone https://github.com/[TODO: repo URL]
```

(Or provide the local path if pre-cloned.)

Then ask:

> "Open one of the skills to read it in a text editor."

This lets you see what skills look like and learn how to use skills made by other people. Use the `/explain-project` skill on a previous agent run to get a layman explanation of what it did (it's okay to start using agents blindly, but you'll get better at using them if you understand how they work).
